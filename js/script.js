...
function setupEventListeners() {
    ...
    if(ELEMENTS.exportXmlBtn) {
        ELEMENTS.exportXmlBtn.addEventListener('click', exportToXML);
    }
}
...
function exportToXML() {
    ...
    STATE.filtered.forEach(med => {
        xml += '  <medicamento>\n';
        xml += `    <codigo>${escapeXML(med.codigo || '')}</codigo>\n`;
        xml += `    <nome>${escapeXML(med.nome || '')}</nome>\n`;
        xml += `    <apresentacao>${escapeXML(med.apresentacao || '')}</apresentacao>\n`;
        xml += `    <categoria>${escapeXML(med.categoria || '')}</categoria>\n`;
        xml += '  </medicamento>\n';
    });
